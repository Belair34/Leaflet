describe('Tooltip', () => {
	...
	describe("autoPan option should pan tooltip into visibility", () => {
		...
		function gettooltipOffset(map, tooltip) {
			...
			const tooltipOffset = tooltip._container.getBoundingClientRect().top;
			return tooltipOffset - mapOffset;
		}

		it("should not pan map to show tooltip content if autoPan is disabled", (done) => {
			map.on('tooltipopen', (e) => {
				const tooltipTopOffset = gettooltipOffset(map, e.tooltip);
